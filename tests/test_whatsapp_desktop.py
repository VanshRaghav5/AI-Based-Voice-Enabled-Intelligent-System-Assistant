...
class TestWhatsAppDesktopKeyFlow:
    ...
    def test_contact_search_variants_do_not_include_quotes(self):
        from backend.tools import whatsapp_desktop as wd
    ...
    def test_open_chat_by_contact_uses_new_chat_keyboard_flow(self):
        ...
        with patch("backend.tools.whatsapp_desktop._focus_and_prime_ui"):
            with patch("backend.tools.whatsapp_desktop._search_contact_in_new_chat") as mock_new_chat:
                with patch("backend.tools.whatsapp_desktop._press_key") as mock_press:
                    with patch("backend.tools.whatsapp_desktop._focus_whatsapp"):
                        with patch("backend.tools.whatsapp_desktop._sleep"):
    ...
    def test_open_chat_by_contact_uses_short_name_variants(self):
        ...
        with patch("backend.tools.whatsapp_desktop._focus_and_prime_ui"):
            with patch("backend.tools.whatsapp_desktop._search_contact_in_new_chat") as mock_search:
                with patch("backend.tools.whatsapp_desktop._press_key"):
                    with patch("backend.tools.whatsapp_desktop._focus_whatsapp"):
                        with patch("backend.tools.whatsapp_desktop._sleep"):
    ...
    def test_search_contact_in_new_chat_opens_picker_and_types_contact(self):
        ...
        with patch("backend.tools.whatsapp_desktop._open_new_chat_dialog") as mock_open:
            with patch("backend.tools.whatsapp_desktop._clear_search_box") as mock_clear:
                with patch("backend.tools.whatsapp_desktop._paste_text") as mock_paste:
                    with patch("backend.tools.whatsapp_desktop._sleep"):
    ...
    def test_send_message_targets_composer_and_sends(self):
        ...
        with patch("backend.tools.whatsapp_desktop._focus_whatsapp") as mock_focus_window:
            with patch("backend.tools.whatsapp_desktop._focus_message_composer") as mock_focus:
                with patch("backend.tools.whatsapp_desktop._paste_text") as mock_paste:
                    with patch("backend.tools.whatsapp_desktop._press_key") as mock_press:
                        with patch("backend.tools.whatsapp_desktop._sleep"):
...
class TestAutomationRouterWhatsApp:
    ...
    def test_send_whatsapp_returns_error_when_send_fails(self):
        from backend.api.controllers import automation_controller as router
...
class TestWhatsAppSendToolSafety:
    def test_send_tool_blocks_when_message_matches_target(self):
        from backend.tools.whatsapp_desktop import WhatsAppSendTool
    ...
    def test_send_tool_returns_structured_feedback_on_wrapped_error(self):
        ...
        tool = WhatsAppSendTool()
        with patch("backend.tools.whatsapp_desktop.send_whatsapp_message", side_effect=Exception("window not found")):
    ...
    def test_message_normalization_removes_target_suffix(self):
        from backend.tools.whatsapp_desktop import _normalize_message_for_target
