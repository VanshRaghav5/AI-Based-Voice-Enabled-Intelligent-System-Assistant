...
class TestPiperTTS:
    ...
    @patch('backend.services.voice.tts.tts_engine.os.remove')
    @patch('backend.services.voice.tts.tts_engine.sd.wait')
    @patch('backend.services.voice.tts.tts_engine.sd.play')
    @patch('backend.services.voice.tts.tts_engine.wav.read')
    @patch('backend.services.voice.tts.tts_engine.subprocess.run')
    @patch('backend.services.voice.tts.tts_engine.os.path.exists')
    ...
    @patch('backend.services.voice.tts.tts_engine.os.path.exists', return_value=False)
...
class TestVoiceCatalog:
    ...
    @patch('backend.services.voice.tts.tts_engine.os.listdir', return_value=['en_US-danny-low.onnx', 'en_GB-jenny-medium.onnx'])
    @patch('backend.services.voice.tts.tts_engine.os.path.isfile', return_value=True)
    @patch('backend.services.voice.tts.tts_engine.os.path.exists', return_value=True)
    def test_get_available_voices_includes_discovered_models(self, mock_exists, mock_isfile, mock_listdir):
        from backend.services.voice.tts.tts_engine import get_available_voices
    ...
    @patch('backend.services.voice.tts.tts_engine.os.path.exists', return_value=True)
    @patch('backend.services.voice.tts.tts_engine.os.listdir', return_value=['en_US-danny-low.onnx'])
    @patch('backend.services.voice.tts.tts_engine.os.path.isfile', return_value=True)
    def test_resolve_voice_model_falls_back_when_voice_missing(self, mock_isfile, mock_listdir, mock_exists):
        from backend.services.voice.tts.tts_engine import _resolve_voice_model
