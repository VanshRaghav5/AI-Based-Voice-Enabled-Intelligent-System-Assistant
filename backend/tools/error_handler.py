...
class ErrorHandler:
    ...
    def handle_error(
        ...
        log(f"Error in {operation}: {error}")
        ...
        if fallback:
            try:
                ...
                log(f"Attempting fallback for {operation}")
                fallback_result = fallback()
                logger.info(f"[{operation}] Fallback succeeded")
                log(f"Fallback succeeded for {operation}")
            except Exception as fallback_error:
                logger.error(f"[{operation}] Fallback failed: {fallback_error}")
                log(f"Fallback failed for {operation}: {fallback_error}")
