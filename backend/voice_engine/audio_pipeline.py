...
def listen_for_gui(duration: float = LISTENING_TIMEOUT) -> str:
    ...
    try:
        ...
        if not text:
            logger.warning("[GUI Listen] Empty transcription — speak clearly into microphone")
            return ""
...
def listen_for_gui_adaptive(should_stop=None) -> str:
    ...
    try:
        ...
        if not text:
            logger.warning("[GUI Listen - Adaptive] Empty transcription — speak clearly into microphone")
            return ""
