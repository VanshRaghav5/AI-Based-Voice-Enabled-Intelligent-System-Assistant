"""
browser_control.py  —  OMINI Browser Action Module
====================================================
...
def _real_profile_dir(browser: str) -> str:
    ...
    fallback = home / ".omini_profiles" / browser
...
class _BrowserSession:
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            profile = _firefox_profile_dir() or str(
                Path.home() / ".omini_profiles" / "firefox"
            )
            kwargs: dict = {
                "headless":    False,
                "slow_mo":     0,
            ...
            try:
                ...
                print(f"[Browser] Firefox real profile failed ({e}), using OMINI profile")
                omini = str(Path.home() / ".omini_profiles" / "firefox_omini")
                Path(omini).mkdir(parents=True, exist_ok=True)
                self._context = await engine_obj.launch_persistent_context(omini, **kwargs)

            await asyncio.sleep(0.5)  # let the browser settle
            # Always open a dedicated OMINI tab instead of grabbing pages[0],
        ...
        if engine_name == "webkit":
            safari_profile = str(Path.home() / ".omini_profiles" / "safari")
        ...
        # 2) OMINI fallback profili
        omini_profile = str(Path.home() / ".omini_profiles" / self.browser_name)
        Path(omini_profile).mkdir(parents=True, exist_ok=True)
        print(f"[Browser] Retrying with OMINI profile: {omini_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(omini_profile, **kwargs)
            await asyncio.sleep(0.5)
            self._page = await self._context.new_page()
            print(f"[Browser] ✅ Launched [{label}] with OMINI profile")
    ...
    async def go_to(self, url: str) -> str:
        ...
        • On PlaywrightTimeout we check whether the URL actually changed;
          if it did, treat it as success so OMINI won't retry endlessly.
    ...
    async def screenshot(self, path: str = None) -> str:
        page = await self._get_page()
        try:
            save_path = path or str(Path.home() / "Desktop" / "omini_screenshot.png")
